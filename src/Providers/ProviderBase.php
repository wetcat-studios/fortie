...
namespace Wetcat\Fortie\Providers;
...
abstract class ProviderBase
{
    ...
    public function getRateLimit()
    {
        if (empty($this->rate_limit)) {
            $this->rate_limit = config(
          'fortie.default.rate_limit',
          config(
              'fortie::default.rate_limit',
              4
          )
    ...
    protected function handleResponse(\GuzzleHttp\Psr7\Response $response)
    {
        ...
        if (in_array('application/xml', $content_type)) {
            $reader = new \Sabre\Xml\Reader();
            $reader->xml($response->getBody());

            return $reader->parse();
        }
        if (in_array('application/pdf', $content_type)) {
